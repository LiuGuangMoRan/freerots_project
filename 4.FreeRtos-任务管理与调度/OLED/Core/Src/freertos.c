...
//实验1-----创建3个任务
    ...
    /* 创建任务Test--动态创建 */
//   xTaskCreate((TaskFunction_t)PlayMusic,"buzzerTask",128,NULL,osPriorityNormal,&xbuzzerTaskHandle);
    ...
    for(;;)
    {
        if(Key_Read() == 1)
        {
            if(xbuzzerTaskHandle == NULL)
            {
               ...
               xTaskCreate((TaskFunction_t)ActiveBuzzer_Test,"buzzerTask",128,NULL,osPriorityNormal,&xbuzzerTaskHandle);
				bRunning = 1;
            }
			else
			{
