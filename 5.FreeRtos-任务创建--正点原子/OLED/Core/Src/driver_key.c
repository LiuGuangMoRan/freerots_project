...
void Key_Init(void)
{
    ...
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    HAL_GPIO_Init(KEY2_GPIO_GROUP, &GPIO_InitStruct);
}
...
{
    uint8_t KeyNum = 0;
    if (HAL_GPIO_ReadPin(KEY_GPIO_GROUP, KEY_GPIO_PIN) == 0)
    {
        vTaskDelay(20);
        while (HAL_GPIO_ReadPin(KEY_GPIO_GROUP, KEY_GPIO_PIN) == 0);
        vTaskDelay(20);
        KeyNum = 1;
    }

    if (HAL_GPIO_ReadPin(KEY2_GPIO_GROUP, KEY2_GPIO_PIN) == 0)
    {
        vTaskDelay(20);
        while (HAL_GPIO_ReadPin(KEY2_GPIO_GROUP, KEY2_GPIO_PIN) == 0);
        vTaskDelay(20);
        KeyNum = 2;
    }
    return KeyNum;
